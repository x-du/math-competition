#!/usr/bin/env python3
"""
Add school names to AMO 2024 results from the USAMO Awardees PDF.
When state is missing, use state from PDF or infer from school name.

Usage: python scripts/add_amo_2024_school_and_state.py [--dry-run]
"""
...
AMO_2024_RESULTS = ROOT / "database" / "contests" / "amo" / "year=2024" / "results.csv"
PDF_PATH = Path("/Users/xiaochendu/Dropbox/Xdu/Patrick/AMO/2024-USAMO-Awardees-FINAL.pdf")
...
SCHOOL_TO_STATE: list[tuple[str | re.Pattern, str, int]] = [
    ...
    ("thayer academy", "Massachusetts", 90),
    ("epiphany", "New York", 85),
    ("englewood academy", "New Jersey", 85),
    ("eo smith", "Connecticut", 85),
    ("iowa city west", "Iowa", 92),
    ("clark high school", "Nevada", 88),
...
def _un_camel(s: str) -> str:
    ...
    # Fix common over-splits from PDF
...
def parse_pdf_awardees() -> tuple[dict[str, str], dict[str, str]]:
    """Parse PDF. Format: Name, School, State/Country, Award (camelCase in 2024)."""
...
def main() -> None:
    ...
    with AMO_2024_RESULTS.open(newline="", encoding="utf-8") as f:
    ...
    with AMO_2024_RESULTS.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames, extrasaction="ignore")
        writer.writeheader()
        writer.writerows(rows)
    print(f"Updated {AMO_2024_RESULTS}")
